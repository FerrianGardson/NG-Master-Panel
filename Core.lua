...
function MasterPanel:OnEnable()
    ...
    if MasterPanel.db.profile.settings.AddNameToEmote then SettingsFrame.AddName:SetChecked(true) else SettingsFrame.AddName:SetChecked(false) end
    if MasterPanel.db.profile.settings.SaveFocus then SettingsFrame.SaveFocusEditbox:SetChecked(true) else SettingsFrame.SaveFocusEditbox:SetChecked(false) end
...
MPA_MainPanel:SetScript("OnDragStart", function(self) self:StartMoving() end)
MPA_MainPanel:SetScript("OnMouseDown", function(self) self:StartMoving() end)
MPA_MainPanel:SetScript("OnMouseUp", function(self) self:StopMovingOrSizing() self:SetUserPlaced(true) end)
...
--------/// Control Buttons UI
-----------------------------------------------------------------------------------------------------------------------------------------------------
MPA_MainPanel.CloseButton = CreateFrame("BUTTON", "MPA_MainPanel.CloseButton", MPA_MainPanel, "MasterPanel:CloseButtonTemplate");
...
MPA_MainPanel.RefreshButton = CreateFrame("BUTTON", "MPA_MainPanel.RefreshButton", MPA_MainPanel, "MasterPanel:RefreshButtonTemplate");
...
elseif EditboxStatement == "LOOKUPCREATURE" then
    if tostring(EditboxText) == "м1" then SendChatMessage(".npc add 28605","WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
	elseif tostring(EditboxText) == "м2" then SendChatMessage(".npc add 11016974","WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
	elseif tostring(EditboxText) == "м3" then SendChatMessage(".npc add 9928816","WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
	elseif tostring(EditboxText) == "м4" then SendChatMessage(".npc add 987663","WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
	elseif tostring(EditboxText) == "ф" then SendChatMessage(".npc add 11002715","WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
    else SendChatMessage(".lo cr "..tostring(EditboxText),"WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player")); end
...
elseif EditboxStatement == "CHATCOLORSTATE" then
    if isempty(tostring(EditboxText)) then
        ...
        if (not MasterPanel.db.profile.settings.ChatRadius) or MasterPanel.db.profile.settings.ChatRadius == nil or tonumber(MasterPanel.db.profile.settings.ChatRadius) == 0 then
            MessageStringSplitter(tostring(EditboxText), 5, 0, tonumber(Chosen_colour))
        else
            MessageStringSplitter(tostring(EditboxText), 5, tonumber(MasterPanel.db.profile.settings.ChatRadius), tonumber(Chosen_colour))
        end
    end
elseif EditboxStatement == "TALKINGHEADSTATE" then
    ...
    elseif isempty(tostring(EditboxText)) then
        ...
        if (not MasterPanel.db.profile.settings.ChatRadius) or MasterPanel.db.profile.settings.ChatRadius == nil or tonumber(MasterPanel.db.profile.settings.ChatRadius) == 0 then
            ...
            TalkingHeadRetranslator(tostring(EditboxText), GetUnitName("target"), GetUnitName("player"), tonumber(MasterPanel.db.profile.settings.ChatRadius))
        end
    end
...
elseif EditboxStatement == "PHASESTATEMENT" then
	...
	elseif string.find(tostring(EditboxText), 1024) then
        print("|cffff9716[ГМ-аддон]: Это фаза для ДМ-ведущих. Тебе туда нельзя.|r")
    elseif string.find(tostring(strlower(EditboxText)), "r") then
		local string nline = tostring(EditboxText):sub(1, -2);
		SendChatMessage(".raidphase "..tonumber(nline),"WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
...
elseif EditboxStatement == "SETCLEARWEATHERSTATEMENT" then
    if not tonumber(EditboxText) or isempty(tostring(EditboxText))	then
        print("|cffff9716[ГМ-аддон]: Номер интенсивности погоды должен быть целым числом в диапазоне 0 - 10.|r")
    elseif tonumber(EditboxText) > 10 or tonumber(EditboxText) < 0 then
        print("|cffff9716[ГМ-аддон]: Номер интенсивности погоды должен быть целым числом в диапазоне 0 - 10.|r")
	else
        SendChatMessage(".weather 1 "..tonumber(EditboxText),"WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
		print("|cffff9716[ГМ-аддон]: Ясная погода установлена. ("..tonumber(EditboxText).. ")|r");
    end
elseif EditboxStatement == "SETRAINWEATHERSTATEMENT" then
    ...
    elseif tonumber(EditboxText) > 10 or tonumber(EditboxText) < 0 then
        ...
        SendChatMessage(".weather 2 "..tonumber(EditboxText),"WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
		print("|cffff9716[ГМ-аддон]: Дождливая погода установлена. ("..tonumber(EditboxText).. ")|r");
    end
elseif EditboxStatement == "SETSNOWWEATHERSTATEMENT" then
    ...
    elseif tonumber(EditboxText) > 10 or tonumber(EditboxText) < 0 then
        ...
        SendChatMessage(".weather 3 "..tonumber(EditboxText),"WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
		print("|cffff9716[ГМ-аддон]: Снежная погода установлена. ("..tonumber(EditboxText).. ")|r");
    end
elseif EditboxStatement == "SETSTORMWEATHERSTATEMENT" then
    ...
    elseif tonumber(EditboxText) > 10 or tonumber(EditboxText) < 0 then
        ...
        SendChatMessage(".weather 4 "..tonumber(EditboxText),"WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
		print("|cffff9716[ГМ-аддон]: Буря установлена. ("..tonumber(EditboxText).. ")|r");
    end
elseif EditboxStatement == "VISUALSPELLSSTATEMENT" then
    if not tonumber(EditboxText) or isempty(tostring(EditboxText)) then
        print("|cffff9716[ГМ-аддон]: Номер визуального спелла должен быть числом.|r")
    else
		local numer = tonumber(EditboxText);
		if numer == 1 then numer = 550000
		elseif numer == 2 then numer = 550004
		elseif numer == 3 then numer = 550019
		elseif numer == 4 then numer = 550024
		elseif numer == 5 then numer = 550010
		elseif numer == 6 then numer = 550021
		elseif numer == 7 then numer = 550045
		elseif numer == 8 then numer = 550028
		elseif numer == 9 then numer = 550043
		elseif numer == 10 then numer = 550051
		elseif numer == 11 then numer = 550002
		elseif numer == 12 then numer = 550003
		elseif numer == 13 then numer = 5500049
		elseif numer == 14 then numer = 550005
		elseif numer == 15 then numer = 550007
		elseif numer == 16 then numer = 5500048
		elseif numer == 17 then numer = 550017
		elseif numer == 18 then numer = 550012
		elseif numer == 19 then numer = 550015
		elseif numer == 20 then numer = 550020
		elseif numer == 21 then numer = 550025
		elseif numer == 22 then numer = 550022
		elseif numer == 23 then numer = 550039
		elseif numer == 24 then numer = 550029
		elseif numer == 25 then numer = 550042
		elseif numer == 26 then numer = 550008
		end
        SendChatMessage(".npccastvisual "..numer,"WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
		print("|cffff9716[ГМ-аддон]: Визуальный эффект установлен на " ..GetUnitName("target").. ".|r");
    end
elseif EditboxStatement == "TOPLAYERSTATEMENT" then
    if isempty(tostring(EditboxText)) or tonumber(EditboxText) then
        ...
        SendChatMessage(".app "..tostring(EditboxText),"WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
    end
elseif EditboxStatement == "NPCEQUIPMENTSTATEMENT" then
    if isempty(tostring(EditboxText)) then
        print("|cffff9716[ГМ-аддон]: Необходимо ввести числа вооружения NPC. Пример: 123 456 789.|r")
    elseif (tonumber(EditboxText) == 0) then
        ...
        SendChatMessage(".changeweapon " ..EditboxText,"WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
    end
elseif EditboxStatement == "SETSCALENPCSTATEMENT" then
    ...
    elseif tonumber(EditboxText) > 5 or tonumber(EditboxText) < 0.05 then
        ...
        SendChatMessage(".mod scale "..tonumber(EditboxText),"WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
    end
elseif EditboxStatement == "NPCPLAYEMOTESTATEMENT" then
    ...
    elseif not tonumber(EditboxText) or isempty(tostring(EditboxText)) then
        ...
        SendChatMessage(".npc playemote "..tonumber(EditboxText),"WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
    end
...
elseif EditboxStatement == "SUMMONSTATEMENT" then
    if isempty(tostring(EditboxText)) then
        if GetUnitName("target")==GetUnitName("player") then print("|cffff9716[ГМ-аддон]: Вы не можете телепортировать самого себя.|r")
		elseif UnitIsPlayer("target") then
		SendChatMessage(".summon "..GetUnitName("target"),"WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player"));
		elseif GetUnitName("target")==nil then
        print("|cffff9716[ГМ-аддон]: Необходимо ввести ник игрока или взять в цель (игрока или npc).|r")
		else SendChatMessage(".npc move","WHISPER" ,GetDefaultLanguage("player"),GetUnitName("player")); end
    elseif tostring(EditboxText) == 1 or string.lower(tostring(EditboxText)) == "all" or string.lower(tostring(EditboxText)) == "рейд" then
    -----
...
MPA_SelectPanel.NPCButton = CreateFrame("Button", "MPA_SelectPanel.NPCButton", MPA_SelectPanel, "MasterPanel:IconButtonBG")
...
MPA_SelectPanel.NPCButton.Icon:SetScript("OnClick", function() MasterPanel:NPCPanel_ShowAll() end)
...
MPA_SelectPanel.NPCButton.Icon.AddLine=colorCode..".makeraid|r позволяет создать рейд тем, кто ниже 10-го уровня";
--- - - - - - - - - - - - -
--- - - - - - - - - - - - -
MPA_SelectPanel.SearchAndDel = CreateFrame("Button", "MPA_SelectPanel.SearchAndDel", MPA_SelectPanel, "MasterPanel:IconButtonBG")  --// First button
MPA_SelectPanel.SearchAndDel:SetPoint("CENTER", -60, -18)

MPA_SelectPanel.SearchAndDel.Icon = CreateFrame("BUTTON", nil, MPA_SelectPanel.SearchAndDel, "MasterPanel:IconButtonTemplate");
...
MPA_SelectPanel.SearchAndDel.Icon:SetScript("OnClick", function() MasterPanel:SearchAndDel_ShowAll() end)
...
--- - - - - - - - - - - - -
--- - - - - - - - - - - - -
MPA_SelectPanel.AurasButton = CreateFrame("Button", "MPA_SelectPanel.AurasButton", MPA_SelectPanel, "MasterPanel:IconButtonBG")  --// First button
MPA_SelectPanel.AurasButton:SetPoint("CENTER", 0, -18)

MPA_SelectPanel.AurasButton.Icon = CreateFrame("BUTTON", nil, MPA_SelectPanel.AurasButton, "MasterPanel:IconButtonTemplate");
...
MPA_SelectPanel.AurasButton.Icon:SetScript("OnClick", function() MasterPanel:AurasAndStates_ShowAll() end)
...
MPA_SelectPanel.Dops.Icon:SetScript("OnClick", function() MasterPanel:ControlPanel_ShowAll() end)
...
MPA_SelectPanel.Dops.Icon.AddLine=colorCode..".npcsave|r позволяет сохранить позицию и анимацию выбранного NPC;|n"..colorCode..".setmount |r садит NPC на маунта;|n"..colorCode..".roam [радиус 0-10]|r заставляет выбранного NPC бродить в указанном радиусе (работает только пока NPC прогружен игроками, потом сбрасывается, а "..colorCode..".unroam|r или "..colorCode..".stoproam|r отменяет это;|n"..colorCode..".follow [Ник]|r заставляет выбранного NPC следовать за игроком, а "..colorCode..".unfollow|r отменяет это;|n"..colorCode..".follow [ник] [дистанция] [угол]|r - дополнительно указывает расстояние на котором держится NPC, а также угол, с какой стороны игрока стоит и идёт NPC. Можно указать r для рандомного числа.";
--- - - - - - - - - - - - -
--- - - - - - - - - - - - -
...
MPA_SelectPanel.Options.Icon:SetScript("OnClick", function() MasterPanel:SettingsFrameOnOff() end)
...
MPA_SelectPanel.Options.Icon.AddLine=colorCode..".reskin|r позволяет изменить расу и ник персонажа;|n"..colorCode..".gps|r показывает координаты места, в котором находится персонаж;|n"..colorCode..".charcust|r показывает параметры внешности персонажа;|n"..colorCode..".tele gm|r перемещает на локацию где можна тестировать ГОшки и NPC;|n"..colorCode..".tele ShowCase|r перемещает в локацию с различными NPC на выбор;|n"..colorCode..".createtargetdisp [название]|r создает Display ID персонажа в таргете;|n"..colorCode..".reloadoutfits|r перезагружает Display ID.";
...
MPA_NPCPanel.NPCSay_Button = CreateFrame("BUTTON", "MPA_MainPanel.NPCSay_Button", MPA_NPCPanel, "MasterPanel:NPCSAYButton");
...
MPA_NPCPanel.NPCEmote_Button = CreateFrame("BUTTON", "MPA_MainPanel.NPCEmote_Button", MPA_NPCPanel, "MasterPanel:NPCEmoteButton");
...
MPA_NPCPanel.Colour_Button = CreateFrame("BUTTON", "MPA_MainPanel.Colour_Button", MPA_NPCPanel, "MasterPanel:NPCColourButton");
...
MPA_NPCPanel.NPCYell_Button = CreateFrame("BUTTON", "MPA_MainPanel.NPCYell_Button", MPA_NPCPanel, "MasterPanel:NPCYellButton");
...
MPA_NPCPanel.TalkingHead_Button = CreateFrame("BUTTON", "MPA_MainPanel.TalkingHead_Button", MPA_NPCPanel, "MasterPanel:NPCTalkingHeadButton");
...
MPA_SearchAndDelPanel.LoCrButton = CreateFrame("BUTTON", "MPA_SearchAndDelPanel.LoCrButton", MPA_SearchAndDelPanel, "MasterPanel:LoCrButtonTemplate");
...
MPA_SearchAndDelPanel.LoCrButton.AddLine="Чтобы создать маунта для морфа введите: "..colorCode.."м1|r (обычный), "..colorCode.."м2|r (быстрый) или "..colorCode.."м3|r (летающий);|n Чтобы создать летающего NPC введите находясь в воздухе: "..colorCode.."ф|r.";
...
MPA_SearchAndDelPanel.LoObjButton = CreateFrame("BUTTON", "MPA_SearchAndDelPanel.LoObjButton", MPA_SearchAndDelPanel, "MasterPanel:LoObjButtonTemplate");
...
MPA_SearchAndDelPanel.LoObjButton.AddLine=colorCode..".movego|r позволяет выделить близжайшую гошку, наклонить и изменить её размер;|n"..colorCode..".gobcopy [радиус] [фаза]|r — копирует (не переносит) объекты в указанном радиусе в указанную фазу;|n"..colorCode..".neardis|r показывает Dysplay ID ближайших ГО;|n"..colorCode..".targetdis|r показывает Dysplay ID ближайшей ГО;|n"..colorCode..".gobnear [guid]|r - гуид объектов;|n"..colorCode..".gobsize [guid] [size]|r - размер объекта;|n"..colorCode..".gob act [guid]|r - действие объекта;|n"..colorCode..".gobtele [guid]|r - перемещает обьект к вам;|n"..colorCode..".gobinfo|r показывает GUID, персонажа, дату установки ближайшей ГО.";
...
MPA_SearchAndDelPanel.DeleteNPC = CreateFrame("Button", "MPA_SearchAndDelPanel.DeleteNPC", MPA_SearchAndDelPanel, "MasterPanel:MiniIconButtonBG")  --// First button
MPA_SearchAndDelPanel.DeleteNPC:SetPoint("CENTER", -90, 0)

MPA_SearchAndDelPanel.DeleteNPC.Icon = CreateFrame("BUTTON", nil, MPA_SearchAndDelPanel.DeleteNPC, "MasterPanel:MiniIconButtonTemplate");
...
MPA_SearchAndDelPanel.DeleteNPC.Icon:SetScript("OnClick", function() MasterPanel:TargetDeleteNPC() end)
...
MPA_SearchAndDelPanel.DeleteNPC.Icon.AddLine=colorCode..".guiddelnpc [GUID]|r позволяет удалить NPC по его GUID;|n"..colorCode..".npc info|r позволяет узнать GUID выбранного NPC. Также вы можете узнать это на сайте в модерке.";

MPA_SearchAndDelPanel.DeleteNearNPC = CreateFrame("Button", "MPA_SearchAndDelPanel.DeleteNPC", MPA_SearchAndDelPanel, "MasterPanel:MiniIconButtonBG")  --// First button
MPA_SearchAndDelPanel.DeleteNearNPC:SetPoint("CENTER", -50, 0)

MPA_SearchAndDelPanel.DeleteNearNPC.Icon = CreateFrame("BUTTON", nil, MPA_SearchAndDelPanel.DeleteNearNPC, "MasterPanel:MiniIconButtonTemplate");
...
MPA_SearchAndDelPanel.UndoGob = CreateFrame("Button", "MPA_SearchAndDelPanel.UndoGob", MPA_SearchAndDelPanel, "MasterPanel:MiniIconButtonBG")  --// First button
MPA_SearchAndDelPanel.UndoGob:SetPoint("CENTER", -10, 0)

MPA_SearchAndDelPanel.UndoGob.Icon = CreateFrame("BUTTON", nil, MPA_SearchAndDelPanel.UndoGob, "MasterPanel:MiniIconButtonTemplate");
...
MPA_SearchAndDelPanel.GobDelRadiusButton = CreateFrame("Button", "MPA_SearchAndDelPanel.GobDelRadiusButton", MPA_SearchAndDelPanel, "MasterPanel:MiniIconButtonBG")  --// First button
MPA_SearchAndDelPanel.GobDelRadiusButton:SetPoint("CENTER", 120, 0)

MPA_SearchAndDelPanel.GobDelRadiusButton.Icon = CreateFrame("BUTTON", nil, MPA_SearchAndDelPanel.GobDelRadiusButton, "MasterPanel:MiniIconButtonTemplate");
...
MPA_SearchAndDelPanel.GobDelRadiusButton.Icon:SetScript("OnClick", function() MasterPanel:WarningFrameShow(Chosen_GobDelRadius) end)
...
MPA_SearchAndDelPanel.GobDelRadiusButton.Icon.AddLine=colorCode..".gob del GUID|r удаляет обьект по его guid.";

MPA_SearchAndDelPanel.GobDelByNameButton = CreateFrame("Button", "MPA_SearchAndDelPanel.GobDelByNameButton", MPA_SearchAndDelPanel, "MasterPanel:MiniIconButtonBG")  --// First button
MPA_SearchAndDelPanel.GobDelByNameButton:SetPoint("CENTER", 80, 0)

MPA_SearchAndDelPanel.GobDelByNameButton.Icon = CreateFrame("BUTTON", nil, MPA_SearchAndDelPanel.GobDelByNameButton, "MasterPanel:MiniIconButtonTemplate");
...
MPA_SearchAndDelPanel.GobDelByNameButton.Icon.AddLine=colorCode..".gob del GUID|r удаляет обьект по его guid.";
...
MPA_AurasAndStatesPanel.Morph = CreateFrame("BUTTON", "MPA_AurasAndStatesPanel.Morph", MPA_AurasAndStatesPanel, "MasterPanel:MorphButtonTemplate");
...
MPA_AurasAndStatesPanel.DeMorph = CreateFrame("BUTTON", "MPA_AurasAndStatesPanel.DeMorph", MPA_AurasAndStatesPanel, "MasterPanel:DeMorphButtonTemplate");
...
MPA_AurasAndStatesPanel.Phase = CreateFrame("BUTTON", "MPA_AurasAndStatesPanel.Phase", MPA_AurasAndStatesPanel, "MasterPanel:PhaseButtonTemplate");
MPA_AurasAndStatesPanel.Phase:SetPoint("CENTER", MPA_MainPanel, "CENTER", 100, 0)
MPA_AurasAndStatesPanel.Phase.Tooltip="Смена фазы";
MPA_AurasAndStatesPanel.Phase.AddLine="Основная фаза: 1;|nДругие фазы: 2, 4, 8, 16, 32, 64;|nДобавьте к номеру фазы малую или большую английскую букву "..colorCode.."R|r чтобы переместить в нужную фазу всю группу или рейд;|n"..colorCode.."3|r - личный телепорт на карту с NPC.|n";
...
MPA_AurasAndStatesPanel.CastVisual = CreateFrame("Button", "MPA_AurasAndStatesPanel.CastVisual", MPA_AurasAndStatesPanel, "MasterPanel:MiniIconButtonBG")  --// First button
MPA_AurasAndStatesPanel.CastVisual:SetPoint("CENTER", 80, -35)

MPA_AurasAndStatesPanel.CastVisual.Icon = CreateFrame("BUTTON", nil, MPA_AurasAndStatesPanel.CastVisual, "MasterPanel:MiniIconButtonTemplate");
...
MPA_AurasAndStatesPanel.CastVisual.Icon:SetScript("OnClick", function()
    if not IsAddOnLoaded("Noble_UI") then
         print("|cffff9716[ГМ-аддон]: Для использования функции включите модификацию '[N] Интерфейс'.|r")
    else
		MasterPanel:EditBoxCollectGarbage();
...
MPA_AurasAndStatesPanel.CastVisual.Icon.AddLine=colorCode.."1|r лед;|n"..colorCode.."2|r огонь;|n"..colorCode.."3|r вода;|n"..colorCode.."4|r молния;|n"..colorCode.."5|r тайная магия;|n"..colorCode.."6|r природа;|n"..colorCode.."7|r свет;|n"..colorCode.."8|r тьма;|n"..colorCode.."9|r скверна;|n"..colorCode.."10|r теневая преграда;|n"..colorCode.."11|r, "..colorCode.."12|r, "..colorCode.."13|r заклинание льда;|n"..colorCode.."14|r, "..colorCode.."15|r, "..colorCode.."16|r заклинание огня;|n"..colorCode.."17|r, "..colorCode.."18|r, "..colorCode.."19|r заклинание арканы;|n"..colorCode.."20|r заклинание природы;|n"..colorCode.."21|r заклинание молнии;|n"..colorCode.."22|r, "..colorCode.."23|r заклинание света;|n"..colorCode.."24|r заклинание тьмы;|n"..colorCode.."25|r заклинание скверны;|n"..colorCode.."26|r ритуал тьмы.";


MPA_AurasAndStatesPanel.StopCastVisual = CreateFrame("Button", "MPA_AurasAndStatesPanel.StopCastVisual", MPA_AurasAndStatesPanel, "MasterPanel:MiniIconButtonBG")  --// First button
MPA_AurasAndStatesPanel.StopCastVisual:SetPoint("CENTER", 120, -35)

MPA_AurasAndStatesPanel.StopCastVisual.Icon = CreateFrame("BUTTON", nil, MPA_AurasAndStatesPanel.StopCastVisual, "MasterPanel:MiniIconButtonTemplate");
...
MPA_AurasAndStatesPanel.SetAura = CreateFrame("Button", "MPA_AurasAndStatesPanel.SetAura", MPA_AurasAndStatesPanel, "MasterPanel:MiniIconButtonBG")  --// First button
MPA_AurasAndStatesPanel.SetAura:SetPoint("CENTER", -120, 0)

MPA_AurasAndStatesPanel.SetAura.Icon = CreateFrame("BUTTON", nil, MPA_AurasAndStatesPanel.SetAura, "MasterPanel:MiniIconButtonTemplate");
...
MPA_AurasAndStatesPanel.SetAura.Icon.AddLine="Небольшой список полезных аур:|n"..colorCode.."1|r - невидимость;";

MPA_AurasAndStatesPanel.ClearAura = CreateFrame("Button", "MPA_AurasAndStatesPanel.ClearAura", MPA_AurasAndStatesPanel, "MasterPanel:MiniIconButtonBG")  --// First button
MPA_AurasAndStatesPanel.ClearAura:SetPoint("CENTER", -80, 0)

MPA_AurasAndStatesPanel.ClearAura.Icon = CreateFrame("BUTTON", nil, MPA_AurasAndStatesPanel.ClearAura, "MasterPanel:MiniIconButtonTemplate");
...
MPA_AurasAndStatesPanel.ClearAura.Icon.AddLine="Выберите цель и нажмите "..colorCode.."ПКМ|r или оставьте поле пустым и нажмите Enter, чтобы убрать все ауры. Вы можете убрать определенную ауру по id.";

MPA_AurasAndStatesPanel.SetScale = CreateFrame("Button", "MPA_AurasAndStatesPanel.SetScale", MPA_AurasAndStatesPanel, "MasterPanel:MiniIconButtonBG")  --// First button
MPA_AurasAndStatesPanel.SetScale:SetPoint("CENTER", 25, 0)

MPA_AurasAndStatesPanel.SetScale.Icon = CreateFrame("BUTTON", nil, MPA_AurasAndStatesPanel.SetScale, "MasterPanel:MiniIconButtonTemplate");
...
MPA_AurasAndStatesPanel.SetScale.Icon.AddLine="Выберите цель и нажмите "..colorCode.."ПКМ|r или оставьте поле пустым и нажмите Enter, чтобы получить стандартный размер.";

-- Время и погода

MPA_AurasAndStatesPanel.SetTimeAndWeatherButton = CreateFrame("Button", "MPA_AurasAndStatesPanel.SetTimeAndWeatherButton", MPA_AurasAndStatesPanel, "MasterPanel:MiniIconButtonBG")  --// First button
MPA_AurasAndStatesPanel.SetTimeAndWeatherButton:SetPoint("CENTER", -25, 0)

MPA_AurasAndStatesPanel.SetTimeAndWeatherButton.Icon = CreateFrame("BUTTON", nil, MPA_AurasAndStatesPanel.SetTimeAndWeatherButton, "MasterPanel:MiniIconButtonTemplate");
...
MPA_SetTimeAndWeatherPanel:Hide()
	...
	MPA_SetTimeAndWeatherPanel.EarlyMorning = CreateFrame("Button", "MPA_SetTimeAndWeatherPanel.EarlyMorning", MPA_SetTimeAndWeatherPanel, "MasterPanel:MiniIconButtonBG")  --// First button
	MPA_SetTimeAndWeatherPanel.EarlyMorning:SetPoint("CENTER", -130, 5)

	MPA_SetTimeAndWeatherPanel.EarlyMorning.Icon = CreateFrame("BUTTON", nil, MPA_SetTimeAndWeatherPanel.EarlyMorning, "MasterPanel:MiniIconButtonTemplate");
	...
	MPA_SetTimeAndWeatherPanel.Morning = CreateFrame("Button", "MPA_SetTimeAndWeatherPanel.Morning", MPA_SetTimeAndWeatherPanel, "MasterPanel:MiniIconButtonBG")  --// First button
	MPA_SetTimeAndWeatherPanel.Morning:SetPoint("CENTER", -90, 5)

	MPA_SetTimeAndWeatherPanel.Morning.Icon = CreateFrame("BUTTON", nil, MPA_SetTimeAndWeatherPanel.Morning, "MasterPanel:MiniIconButtonTemplate");
	...
	MPA_SetTimeAndWeatherPanel.Noon = CreateFrame("Button", "MPA_SetTimeAndWeatherPanel.Noon", MPA_SetTimeAndWeatherPanel, "MasterPanel:MiniIconButtonBG")  --// First button
	MPA_SetTimeAndWeatherPanel.Noon:SetPoint("CENTER", -50, 5)

	MPA_SetTimeAndWeatherPanel.Noon.Icon = CreateFrame("BUTTON", nil, MPA_SetTimeAndWeatherPanel.Noon, "MasterPanel:MiniIconButtonTemplate");
	...
	MPA_SetTimeAndWeatherPanel.Evening = CreateFrame("Button", "MPA_SetTimeAndWeatherPanel.Evening", MPA_SetTimeAndWeatherPanel, "MasterPanel:MiniIconButtonBG")  --// First button
	MPA_SetTimeAndWeatherPanel.Evening:SetPoint("CENTER", -10, 5)

	MPA_SetTimeAndWeatherPanel.Evening.Icon = CreateFrame("BUTTON", nil, MPA_SetTimeAndWeatherPanel.Evening, "MasterPanel:MiniIconButtonTemplate");
	...
	MPA_SetTimeAndWeatherPanel.LateEvening = CreateFrame("Button", "MPA_SetTimeAndWeatherPanel.LateEvening", MPA_SetTimeAndWeatherPanel, "MasterPanel:MiniIconButtonBG")  --// First button
	MPA_SetTimeAndWeatherPanel.LateEvening:SetPoint("CENTER", 30, 5)

	MPA_SetTimeAndWeatherPanel.LateEvening.Icon = CreateFrame("BUTTON", nil, MPA_SetTimeAndWeatherPanel.LateEvening, "MasterPanel:MiniIconButtonTemplate");
	...
	MPA_SetTimeAndWeatherPanel.Night = CreateFrame("Button", "MPA_SetTimeAndWeatherPanel.Night", MPA_SetTimeAndWeatherPanel, "MasterPanel:MiniIconButtonBG")  --// First button
	MPA_SetTimeAndWeatherPanel.Night:SetPoint("CENTER", 70, 5)

	MPA_SetTimeAndWeatherPanel.Night.Icon = CreateFrame("BUTTON", nil, MPA_SetTimeAndWeatherPanel.Night, "MasterPanel:MiniIconButtonTemplate");
	...
	MPA_SetTimeAndWeatherPanel.Clear = CreateFrame("Button", "MPA_SetTimeAndWeatherPanel.Clear", MPA_SetTimeAndWeatherPanel, "MasterPanel:MiniIconButtonBG")  --// First button
	MPA_SetTimeAndWeatherPanel.Clear:SetPoint("CENTER", -130, -35)

	MPA_SetTimeAndWeatherPanel.Clear.Icon = CreateFrame("BUTTON", nil, MPA_SetTimeAndWeatherPanel.Clear, "MasterPanel:MiniIconButtonTemplate");
	...
	MPA_SetTimeAndWeatherPanel.Clear.Icon.AddLine="Применяется к текущей цели или ко всем в группе/рейде, если цель не выбрана.";
	
	MPA_SetTimeAndWeatherPanel.Rain = CreateFrame("Button", "MPA_SetTimeAndWeatherPanel.Rain", MPA_SetTimeAndWeatherPanel, "MasterPanel:MiniIconButtonBG")  --// First button
	MPA_SetTimeAndWeatherPanel.Rain:SetPoint("CENTER", -90, -35)

	MPA_SetTimeAndWeatherPanel.Rain.Icon = CreateFrame("BUTTON", nil, MPA_SetTimeAndWeatherPanel.Rain, "MasterPanel:MiniIconButtonTemplate");
	...
	MPA_SetTimeAndWeatherPanel.Rain.Icon.AddLine="Применяется к текущей цели или ко всем в группе/рейде, если цель не выбрана.";

	MPA_SetTimeAndWeatherPanel.Snow = CreateFrame("Button", "MPA_SetTimeAndWeatherPanel.Snow", MPA_SetTimeAndWeatherPanel, "MasterPanel:MiniIconButtonBG")  --// First button
	MPA_SetTimeAndWeatherPanel.Snow:SetPoint("CENTER", -50, -35)

	MPA_SetTimeAndWeatherPanel.Snow.Icon = CreateFrame("BUTTON", nil, MPA_SetTimeAndWeatherPanel.Snow, "MasterPanel:MiniIconButtonTemplate");
	...
	MPA_SetTimeAndWeatherPanel.Snow.Icon.AddLine="Применяется к текущей цели или ко всем в группе/рейде, если цель не выбрана.";

	MPA_SetTimeAndWeatherPanel.Storm = CreateFrame("Button", "MPA_SetTimeAndWeatherPanel.Storm", MPA_SetTimeAndWeatherPanel, "MasterPanel:MiniIconButtonBG")  --// First button
	MPA_SetTimeAndWeatherPanel.Storm:SetPoint("CENTER", -10, -35)

	MPA_SetTimeAndWeatherPanel.Storm.Icon = CreateFrame("BUTTON", nil, MPA_SetTimeAndWeatherPanel.Storm, "MasterPanel:MiniIconButtonTemplate");
	...
	MPA_SetTimeAndWeatherPanel.Storm.Icon.AddLine="Применяется к текущей цели или ко всем в группе/рейде, если цель не выбрана.";
...
MPA_NPCPosPanel:Hide()
	...
	MPA_NPCPosPanel.SomeSleep = CreateFrame("Button", "MPA_NPCPosPanel.SomeSleep", MPA_NPCPosPanel, "MasterPanel:MiniIconButtonBG")  --// First button
	MPA_NPCPosPanel.SomeSleep:SetPoint("CENTER", -50, 5)

	MPA_NPCPosPanel.SomeSleep.Icon = CreateFrame("BUTTON", nil, MPA_NPCPosPanel.SomeSleep, "MasterPanel:MiniIconButtonTemplate");
	...
	MPA_NPCPosPanel.TurnWeapon = CreateFrame("Button", "MPA_NPCPosPanel.TurnWeapon", MPA_NPCPosPanel, "MasterPanel:MiniIconButtonBG")  --// First button
	MPA_NPCPosPanel.TurnWeapon:SetPoint("CENTER", -130, -35)

	MPA_NPCPosPanel.TurnWeapon.Icon = CreateFrame("BUTTON", nil, MPA_NPCPosPanel.TurnWeapon, "MasterPanel:MiniIconButtonTemplate");
	...
	MPA_NPCPosPanel.SavePos = CreateFrame("Button", "MPA_NPCPosPanel.SavePos", MPA_NPCPosPanel, "MasterPanel:MiniIconButtonBG")  --// First button
	...
	MPA_NPCPosPanel.Ground = CreateFrame("Button", "MPA_NPCPosPanel.Ground", MPA_NPCPosPanel, "MasterPanel:MiniIconButtonBG")  --// First button
...
MPA_ControlPanel.EmoteST = CreateFrame("Button", "MPA_ControlPanel.EmoteST", MPA_ControlPanel, "MasterPanel:MiniIconButtonBG")
MPA_ControlPanel.EmoteST:SetPoint("CENTER", 80, 5)

MPA_ControlPanel.EmoteST.Icon = CreateFrame("BUTTON", nil, MPA_ControlPanel.EmoteST, "MasterPanel:MiniIconButtonTemplate");
...
MPA_ControlPanel.EmoteStatement = CreateFrame("Button", "MPA_ControlPanel.EmoteStatement", MPA_ControlPanel, "MasterPanel:MiniIconButtonBG")
MPA_ControlPanel.EmoteStatement:SetPoint("CENTER", 120, 5)

MPA_ControlPanel.EmoteStatement.Icon = CreateFrame("BUTTON", nil, MPA_ControlPanel.EmoteStatement, "MasterPanel:MiniIconButtonTemplate");
...
MPA_ControlPanel.RollButton = CreateFrame("BUTTON", "MPA_ControlPanel.RollButton", MPA_ControlPanel, "MasterPanel:BattleButtonTemplate");
MPA_ControlPanel.RollButton:SetPoint("CENTER", MPA_MainPanel, "CENTER", 5, 0)
MPA_ControlPanel.RollButton.Tooltip="Панель боёвки";
MPA_ControlPanel.RollButton.AddLine="Клавиша "..colorCode.."ЛКМ|r чтобы открыть/закрыть окно боевки;|nКлавиша "..colorCode.."ПКМ|r чтобы открыть/закрыть окно баффов и дебаффов;|nНажмите "..colorCode.."СКМ|r чтобы заблокировать/разблокировать передвижение панели боевки. При блокировании, панель автоматически переместится к окну аддона.";
...
MPA_ControlPanel.Summon = CreateFrame("Button", "MPA_ControlPanel.Summon", MPA_ControlPanel, "MasterPanel:MiniIconButtonBG")  --// First button
...
MPA_ControlPanel.Summon.Icon.AddLine="Суммон выбранной цели (игрока или npc). Если таргета нет - суммон по нику.|nЕсли ввести цифру "..colorCode.."1|r или слова "..colorCode.."рейд|r, "..colorCode.."all|r - будет суммон ко всему рейду.";


-- Raid summon

MPA_ControlPanel.RaidSum = CreateFrame("Button", "MPA_ControlPanel.RaidSum", MPA_ControlPanel, "MasterPanel:MiniIconButtonBG")  --// First button
MPA_ControlPanel.RaidSum:SetPoint("CENTER", 5, -35)

MPA_ControlPanel.RaidSum.Icon = CreateFrame("BUTTON", nil, MPA_ControlPanel.RaidSum, "MasterPanel:MiniIconButtonTemplate");
...
MPA_ControlPanel.ToPlayer = CreateFrame("Button", "MPA_ControlPanel.ToPlayer", MPA_ControlPanel, "MasterPanel:MiniIconButtonBG")  --// First button
MPA_ControlPanel.ToPlayer:SetPoint("CENTER", 40, -35)

MPA_ControlPanel.ToPlayer.Icon = CreateFrame("BUTTON", nil, MPA_ControlPanel.ToPlayer, "MasterPanel:MiniIconButtonTemplate");
...
MPA_ControlPanel.NPCEquipment = CreateFrame("Button", "MPA_ControlPanel.NPCEquipment", MPA_ControlPanel, "MasterPanel:MiniIconButtonBG")  --// First button
MPA_ControlPanel.NPCEquipment:SetPoint("CENTER", 80, -35)

MPA_ControlPanel.NPCEquipment.Icon = CreateFrame("BUTTON", nil, MPA_ControlPanel.NPCEquipment, "MasterPanel:MiniIconButtonTemplate");
...
MPA_ControlPanel.PossUnposs = CreateFrame("BUTTON", "MPA_ControlPanel.PossUnposs", MPA_ControlPanel, "MasterPanel:PossUnpossButton");
...
MPA_ControlPanel.Waypoint = CreateFrame("BUTTON", "MPA_ControlPanel.Waypoint", MPA_ControlPanel, "MasterPanel:WaypointButtonTemplate");
MPA_ControlPanel.Waypoint:SetPoint("CENTER", MPA_MainPanel, "CENTER", -100, 0)
MPA_ControlPanel.Waypoint.Tooltip="Waypoints";
MPA_ControlPanel.Waypoint.AddLine="Позволяет настраивать маршруты для NPC.|n"..colorCode..".taxi create|r - новый полет;|n"..colorCode..".taxi add ID|r - добавить точку полета;|n"..colorCode..".taxi save ID IdNpc|r - сохранить полет на NPC;|n"..colorCode..".taxi go|r - отправить игрока в полет (он должен быть рядом с начальной точкой).";
...
SettingsFrame.CloseButton = CreateFrame("BUTTON", "SettingsFrame.CloseButton", SettingsFrame, "MasterPanel:CloseButtonTemplate");
...
SettingsFrame.NPCSay:SetScript("OnClick", 
  function()
    ...
    if MasterPanel.db.profile.settings.NPCSayByEmote then SettingsFrame.NPCSay:SetChecked(true) else SettingsFrame.NPCSay:SetChecked(false) end
  end
);
